...
- **Crossover:** the first future hour (≤ `PREDICT_HORIZON_H`) where `inside_proj > COMFORT_HIGH` **and** `out_corr ≤ inside_proj − OPEN_MARGIN` is the predicted open time; it closes again when `out_corr ≥ inside_proj − CLOSE_MARGIN` → `open_intervals`. No crossover today → "vandaag dicht houden". **`currently_open`:** this crossover search only ever encodes the plain "cool" condition — it doesn't know about a dead-band hold, banking, dehumidify or fresh-air open. Without correction, a room already open for one of those other reasons showed a gap on the per-room timeline (`docs/window.html`) until the next *plain-cool* crossover, contradicting the OPEN chip right above it. `predict_open_intervals(..., currently_open=advice=="open")` forces the very first (in-range) grid point open in that case, so the timeline segment always starts at "now" when the room is actually open — the rest of the projection is unchanged (same static-approximation limitation as the dehumidify/fresh-air triggers not being projected forward).

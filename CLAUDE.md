...
- **Each zone is a single well-mixed node — no intra-zone vertical stratification.** This matters most for the stairwell (`stair`): its `predicted_temp` is the **volume-average of the whole shaft, not the top or the bottom**. Height enters only the *airflow* — the stack term (`Pa_eff = P − ρ·g·z` in `solve_network`) uses each opening's `center_height_m` + each zone's temperature-dependent density, so the model knows *which way* air moves through the shaft — but that air is then dumped into one node, so the warm air that physically pools at the top of the trapkoker is averaged straight back into the cool air at the bottom. A per-floor sub-zone split (stratification would emerge for free from the same stack + advection physics, ~all in `house_model.json`) was **considered and deliberately declined**: there is no tado sensor in the shaft to validate sub-zone temps against, and it would re-wire how the floors couple and shift the calibrated rooms (office/hotties/ted). The dashboard stair cell is labelled accordingly (`gem. hele koker`).

...
- `.github/workflows/station-accuracy.yml` — **manual-dispatch only** (no cron), inputs `days` + `notify`, `permissions: contents: write` (commits the dashboard JSON **+ de shards**), checkout gepind op de branch-tip (het archief mag niet vanaf een oudere momentopname gecommit worden)

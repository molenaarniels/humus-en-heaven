...
This repo contains eleven independent automation pipelines, all running on GitHub Actions (most notify via Telegram; Project 7's Telegram is optional and Project 8 has none), plus one manual/incidental analysis script (Project 12, no GitHub Action). They share Telegram/WU/Gist secrets and a few **deliberate read-only** data hand-offs (Project 5 reads Project 1's `data.json`; Project 8 reads Project 6's `window_data.json`; Projects 9/10 reuse Project 8's pure helpers + artefacts read-only; Project 11 aggregates the published artefacts of 1/5/7/8; Project 12 reuses Project 8's pure helpers + artefacts read-only), but are otherwise separate. The daily jobs are fired at their local target time by the **Timing Orchestrator** (`.github/workflows/orchestrator.yml`, a self-driven 15-min klok-loop); each project keeps a ~30–60 min later fallback cron + a dedup guard-job.
...
## Project 12: Isolatie-analyse (House Insulation / Heat-Loss Estimator)

**Goal:** Per room, estimate insulation quality (a warmteverlies-coëfficiënt, UA in W/K) from a year of tado history, cross-check it against Project 8's continuous calibration, and surface a qualitative "which room, and why" narrative — no continuous pipeline, **incidental/manual only**.

### Files
- `insulation_analysis.py` — parses a local tado `dayReport`-export, fetches the matching Open-Meteo ERA5 archive window (one call, any date range — the same endpoint `station_accuracy.py` uses), fits a per-room free-float thermal-decay regression, cross-checks against `docs/airflow_learned.json`, and writes `docs/insulation_data.json`
- `docs/insulation.html` + `docs/js/insulation.js` — dashboard: per-room UA ranking, geometry-based "why" narrative, a coarse monthly trend
- `docs/insulation_data.json` — **generated by the script, aggregated-only, never contains raw/per-timestamp tado data**
- **No GitHub Actions workflow.** This is a manual, occasional script — run locally (or in a Claude Code session) whenever a fresh tado export is available, not on a cron. The user occasionally uploads a fresh export; there's no pipeline that fetches it automatically.

### Method
Project 8's 48h online calibration can't reliably identify every channel (e.g. `office.ua_env`/`ua_roof`/`f_air` sit railed at their bounds — a known saturation tell). A full year of tado history gives far more seasonal ΔT/solar diversity, but reusing Project 8's Gauss-Newton network calibrator at that scale is prohibitively expensive (~34,000 full-timeline `simulate()` passes). Instead:
- Only hours where tado's `callForHeat == "NONE"` (heating genuinely off) are used — this sidesteps ever needing to know actual radiator wattage.
- Per room: `dT_in/dt ≈ k·(T_out − T_in) + s·I_solar,room + c`, fit via ordinary least squares (a 3×3 normal-equations solve, reusing `airflow_model.py`'s pure-Python `solve_linear` — no numpy, matching the repo's stdlib-only convention). If the solar column is degenerate (e.g. no solar variation in the input), falls back to a 2-variable fit (`k`, `c`) rather than failing outright (`solar_dropped` flag).
- `k` (h⁻¹) is the primary, assumption-free metric. `UA` (W/K) is derived via `UA = k/3600 · C_effective`, reusing Project 8's geometry-based capacitance prior (`room_base_capacitances` from `airflow_model.py`) — labeled as depending on that assumption.
- Per-room solar uses Project 8's pure helpers read-only (`load_house`, `sun_position`, `facade_irradiance`, `per_window_solar`) at each window's *default* shading state — there's no year-long openings log, so this is a documented approximation.
- Cross-checked against Project 8's online `ua_env`(+`ua_roof`) for the same room (`docs/airflow_learned.json`, read-only) — agreement/disagreement is reported, with `office` called out specifically since its 48h fit is the least trustworthy there.
- The "why" narrative cross-references `house_model.json` geometry (window/wall ratio, dominant facade orientation, floor, roof exposure) — the physics model has no per-surface (window-vs-wall) U-value, so this stays room-level, not per-surface.

### Privacy — never commit the raw tado export
This repo is **public**. A tado `dayReport` export reveals occupancy (HOME/AWAY) not just via its explicit stripes but via the indoor-temperature trace itself (heat-saving setback dips when the house is empty) — stripping fields isn't enough. **The raw export is never read from, or written to, any path inside the git working tree**, and is never committed in any form, stripped or not. Only `docs/insulation_data.json` is committed, and it contains **aggregated results only**: per-room fitted parameters, rankings, coverage counts, and — for any time-series chart — **monthly** aggregates, never daily/hourly. Room coverage is partial and grows over time as more room exports are provided occasionally (a subset of `living`/`ted`/`hotties`/`office`/`bath` — `stair` has no tado sensor and is always excluded, same as Project 8).

### Relation to other projects
Read-only on Project 8: reuses its pure helpers (`load_house`, `sun_position`, `facade_irradiance`, `per_window_solar`, `room_base_capacitances`, `solve_linear`, `load_learned`, `merged_params`) and reads `docs/airflow_learned.json` + `house_model.json` — same read-only reuse pattern as Projects 9/10. Never writes any Project 8 artefact. No secrets, no Telegram, no GitHub Action.
...
- **Never commit a raw tado history export** (Project 12's input), or any per-timestamp
  derivation of one — it reveals occupancy via the temperature trace itself, not just its
  explicit stripes, and this repo is public. Only `docs/insulation_data.json`'s aggregated
  output (per-room fitted parameters, rankings, monthly-not-daily aggregates) may be committed

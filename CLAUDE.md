...
- **Buur-anker gekapt (`NEIGHBOR_TRANSFORM = "cap23"`, `neighbor_anchor`)**: het party-muur-anker wordt op 23°C geklemd — burenbinnenkant volgt in een hittegolf het 3-daags buitengemiddelde (24–26°C) niet. Uitkomst van de **train-and-set-campagne (juli 2026, `tools/twin2_experiment.py`)**: een 3-voudige 0-epoch-configtest (9 niet-overlappende 5d-vensters, elk precies één keer held-out, huidige anker-params) — wiskundig inert zolang het gemiddelde < 23°C (8/9 vensters Δ0.00) en −0.29°C op het hittegolf-venster. Verworpen op dezelfde toets: subzones strippen, Td-seed uit de kamer-startmeting, geïnverteerd slaapkamer-gain-profiel, 48u warmup, gedempt anker. Methodologische les uit de twee refit-screeningsrondes: **hertrainen op een deel-set overfit onherroepelijk** (6 vensters × 65 params: train ~0.9°C, held-out 1.3+°C, óók de A/A-jitter-controle) — arm-vergelijking dus altijd 0-epoch/param-compatibel, en het anker alleen laten evolueren via het bewezen graduele volle-data-regime (dat generaliseert: 0.98 in-sample → 1.008 held-out).

...
- **Batch-anker i.p.v. checkpoint/fallback**: `--batch` fit (mini-batch Gauss-Newton: 5-daagse vensters, stride 3d, 24u warmup elk, Huber zonder recency, ridge naar de kale priors, `BATCH_TIME_BUDGET_S`; **warm-start vanaf het vorige rev-passende anker** (`batch_start_params`) — één epoch kost ~15–20 min dus het budget laat maar ~2 stappen per run toe; doorstarten maakt de wekelijkse batches cumulatief) schrijft `docs/airflow2_batch.json`; de kwartierrun adopteert een nieuwer, rev-passend anker éénmalig (`ANCHOR_BLEND` 0.5, stempels `anchor_at`/`anchor_src`, `anchored`-markering op het leercurve-punt) en de online ridge trekt voortaan naar het anker i.p.v. de priors. Twin 1's checkpoint/auto-fallback-mechaniek is bewust weggelaten (veroorzaakte daar tweemaal een fallback-lus; het anker vervult de vangnet-rol), net als `backfill_rmse_history` (v1 — twin 1's curve blijft de geheelde referentie).

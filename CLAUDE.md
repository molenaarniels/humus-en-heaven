...
- `tools/night_anchor_experiment.py` — **held-out harnas voor de anker-stap**: replay van het productiepad over de maand-shards (één virtuele 18:45 per avond, echte gerapporteerde log in de forecast-fase zodat het residu aan het ánker toe te schrijven is), gescoord per horizon-emmer + per nacht gepaard. De anker-stap is de grootste én stilste foutbron in deze voorspelling — elke variant draait gewoon door, alleen de nachtcurve staat er structureel naast — dus varianten horen hiermee gemeten te worden i.p.v. beredeneerd. Levert de getallen die in `anchor_mass_now` geciteerd staan; read-only, geen netwerk, deterministisch.
...
- **Massaknoop mee-ijken (`anchor_mass_now`, juli 2026).** De 24u-warmup is een blinde sim en drijft weg (gemeten: de luchtknoop stond bij aankomst gemiddeld 0,83 °C naast). `anchor_now` repareerde alleen de **lucht**; de massaknoop hield die drift vast, en omdat die veruit de meeste capaciteit draagt trok hij de zojuist geijkte lucht er binnen een paar uur weer doorheen — de fout werd daardoor een bijna zuivere offset over de hele nacht (RMSE ≈ |bias|). Dít was veruit de grootste foutbron in deze voorspelling, niet de fysica: op 9 held-out nachten **RMSE 1,46 → 0,86 °C**, bias −1,43 → −0,83. Het verklaart ook waarom de fysica-parameters niets uithaalden (`c_mass` ×4 verschoof de bias 0,08°, `ua_env` ×0,5 slechts 0,03°): het verlies ging niet naar buiten maar naar een te koude ínterne massa. Kamers zonder metingen houden hun sim-waarde (fail open). Warmup-`tm_seed` uit metingen is géén alternatief: over 24u drijft het model gewoon opnieuw weg (gemeten 1,34).
  - **…en de schatter is sinds aug 2026 de starre schuif, niet meer het gedempte gemiddelde.** Het oorspronkelijke anker schatte de massa lós van de sim (EWMA van de gemeten kamerlucht, `MASS_ANCHOR_TAU_H` 8u) en gooide daarmee het gemodelleerde **Tm−Ta-verschil** weg — terwijl dát juist echte fysica is (gemeten: de massa loopt om 18:45 ~0,2 °C áchter op de lucht na een warme dag). Lucht en massa zijn via `H_am` gekoppeld en drijven dus *sámen* weg: de drift is een gemeenschappelijke fout, en de zuivere correctie is beide knopen over dezelfde δ opschuiven (`anchor_delta` → `anchor_mass_now(air_delta=…)`). Alleen de lucht ijken laat de massa ~0,6 °C inconsistent achter, waarna diezelfde koppeling de lucht meteen weer van de meting af trekt. **De terugval blijft staan waar het luchtanker niét vuurde** (meting ontbreekt of ouder dan `ANCHOR_MAX_STALENESS_MIN`): dán is er geen betrouwbare δ, en een schuif van 0 zou de massa op zijn weggedreven warmup-waarde laten staan — vrijloop, de slechtste optie. Precies die kamer-nachten (13 van de 66 voor ted) waren de hele reden dat de kále schuif op ted eerst slechter leek.
  - **Gemeten** (`tools/night_anchor_experiment.py`, 66 held-out nachten uit de shards, replay van het productiepad tegen de echte tado-samples; alle kamers gepoold, RMSE °C): geen massa-anker 1,45 · alleen het gedempte gemiddelde 1,14 · alleen de starre schuif 1,05 · **de combinatie 1,04** (ted zelf 0,97 → 0,95). Gesplitst: waar het luchtanker vuurde 1,09 → **0,97**, waar het niet vuurde 1,42 (terugval, ongewijzigd) tegen 1,51 voor de kále schuif. Per nacht gepaard −0,100 ± 0,018 °C (t = −5,5). Het scherpst op korte horizon, waar de inconsistentie het hardst aankomt: op h≈1u 1,19 (alleen lucht) → 0,73 (gedempt gemiddelde) → **0,52**. Bewust nog steeds **niet** `tm = ta` (de instantane geijkte lucht): dat past over de volle nacht nóg iets beter (0,99 gepoold) maar is fysisch een overschatting, en het wint juist doordát een te warme massa de resterende koud-bias van het model wegstreept — twee fouten tegen elkaar wegstrepen is precies wat de om_bias-revisie aan het opruimen was. De starre schuif haalt die winst wél zuiver, en is op h≈1u simpelweg de beste arm (0,52 vs 0,56).
...
- `.github/workflows/vent-notify.yml` — self-driven quarter-hour loop (cron-kick `9,29,49` —
...
- `docs/vent.html` + `docs/js/vent.js` — dashboard: **meldmodal** (writes `house_openings.json`
  ...
  huis-brede pauze-toggle), plattegrond met stroompijlen, kamerkaarten, voorspeld-vs-gemeten,
  leercurve, speeltuin (gedeelde `speeltuin.js`).
- `docs/vent_data.json`, `docs/vent_learned.json` — **generated by the action, never edit manually**.
...
7. `simulate()` → write + commit `docs/vent_data.json` + `docs/vent_learned.json`; append fresh samples/weather hours to the shards.

### Physics (samenvatting — do not casually retune the structure)
Twin 1's validated physics, ported verbatim into `vent_physics.py`: multi-zone druknetwerk (Newton + gedempte herkansing, laminair/turbulent orifice-regime, één wind-referentiehoogte per gevel), eenzijdige ventilatie (de Gids & Phaff), 2-knoops RC per kamer met de tussenwoning-termen (party-muren met gekapt buur-anker, interne gains, dak-sol-air, interzone-geleiding, bodemkoppeling), trap-stratificatie (gemeten-γ-gradiënt + Brown–Solvason-deur-counterflow), sensor-plaatsing-bias als meet-laag, hoekafhankelijk glas (beam-IAM), en de om_bias-ontbiaste buiten-driver + wu_bias-verfijnde nu-meting. **`PHYSICS_REV` 6** stempelt de learned state (mismatch → params terug naar de priors + anomalie-cooloff; daarna her-seeden via `tools/vent_seed.py`). De volledige meetgeschiedenis achter deze termen staat in `AIRFLOW_ASSESSMENT.md` / `AIRFLOW2_ASSESSMENT.md`.
...
`vent_data.json`: `generated_at`, `as_of_local`, `source: "vent_twin"`, `model_version`, `weather` (outside temp/RH + `outside_source`, wind/gust/shortwave, sun az/el, `neighbor_temp`, `ground_temp`, `wu_solar_scale`), `openings`, `controls`, `paused`/`paused_since`, `ac` (`room`, `rooms[]`), `rooms.<id>` (`label`, `predicted_temp` (sensor-ruimte), `predicted_air_temp`, `predicted_mass_temp`, `actual_temp`, `error`, `ach`, `solar_w`, `solar_by_window`, `env_w`, `vent_w`, `trend_c_per_h`, `humidity`, `comfort_low/high`, `ac`, `heating`, `paused`, `sensor_outdoor_frac`, `predicted_series[]`, `actual_series[]`, `params`, plus de stair-velden op de stratify-zone: `stair_gradient_c_per_m`/`stair_crown_c`/`stair_pin_error_c`/`stair_gamma_w`/`predicted_temp_top`/`predicted_temp_bottom`), `flows[]` (`{id, a, b, flow_m3s}`), `learned` (`params`, `rmse`, `rmse_naive`, `skill`, `railed`, `rmse_history`, `held`, `paused`), `house_meta` (volledige speeltuin-geometrie). `vent_learned.json`: `updated_at`, `model_version`, `physics_rev`, `params`, `rmse`, `skill`, `railed`, `rmse_history[]` (punten: `t`/`rmse`/`held`/`paused`/`version` + additief `skill`/`rmse_naive`/`wx`), `anomaly` (`held_since`/`cooloff_until`/`nudged_at`/`escaped_at`), `seed_src` (van `tools/vent_seed.py`, zolang er nog geen leercurve is).

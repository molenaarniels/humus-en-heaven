...
- **2-node RC thermal** (`simulate`): per zone an air node `T_a` (fast) + wall/furniture mass node `T_m` (slow), integrated **implicitly** (backward Euler → one linear solve per substep via `solve_linear`; explicit Euler blows up under strong door coupling). Heat terms: ventilation `ρcp·Q_fresh·(T_out−T_a)`, advective door coupling between rooms, envelope conduction, air↔mass coupling, and solar `Σ τ·shading·I_window·glass_m2` split `f_air` air / `1−f_air` mass.
...
`airflow_data.json`: `generated_at`, `as_of_local`, `source`, `weather` (outside temp/RH, wind speed/dir/gust, sun az/el), `openings`, `controls` (elements + current state, for the modal), `rooms.<id>` (`actual_temp`, `predicted_temp`, `predicted_mass_temp`, `error`, `ach`, `solar_w`, `humidity`, `comfort_low/high`, `predicted_series[]`, `actual_series[]`, `params`), `flows[]` (per-opening m³/s for the floor-plan), `suggestion` (`instructions[]`, `ranked[]`, `keep_closed`, `headline`), `learned` (`params`, `rmse`, `rmse_history`), `house_meta`.
